...
import {Container, Owner, FilterList, Loading, BackButton, IssuesList, PageActions} from './styles';
    ...
    useEffect(()=>{
        async function load(){
            ...
            const [repositorioData, issuesData] = await Promise.all([
                ...
                {
                    params:{
                        state: filters.find(f => f.active).state, //começa como all
                        per_page: 5
                    }
                })
            ]);
            ...
            <IssuesList>
                {issues.map(issue =>(
                    <li key={String(issue.id)}>
                        ...
                        <div>
                            <strong>
                                <a href={issue.html_url}>{issue.title}</a>{/*a instead Link because it is an external ref */}
